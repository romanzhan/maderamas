...
const FLOWS = {
  ...
  content: { dir: 'content', sizes: [480, 960, 1600, 1920], budgetsKb: {} },
...
async function processFlow(type, flow) {
  ...
  for (const file of readdirSync(dir)) {
    ...
    // Исходник шире самого крупного подошедшего размера (816 при шкале 480/960/1600):
    // добавляем его собственную ширину, иначе половина присланных пикселей пропадает,
    // а на крупном экране картинка выглядит мылом
    const biggest = sizes[sizes.length - 1]
    if (biggest && usable > biggest * 1.1) sizes.push(usable)
